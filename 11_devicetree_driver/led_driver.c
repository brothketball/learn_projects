...
static int led_init(void)
{
	...
	*/
	myprintk("hello world\r\n");
	test_device_node = of_find_node_by_path("/test");
	if (test_device_node == NULL) {
		myprintk("test_device_node is error\r\n");
		return -1;
	}
	myprintk("test_device_node->name is %s\r\n", test_device_node->name);

	test_node_property = of_find_property(test_device_node, "compatible", &size);
	if (test_node_property == NULL) {
		myprintk("test_node_property is err\r\n");
		return -1;
	}
	myprintk("test_node_property->name is %s\r\n", test_node_property->name);
	myprintk("test_node_property->value is %s\r\n", (char *)test_node_property->value);

	ret = of_property_read_u32_array(test_device_node, "reg", reg_values, 9);
	if (ret < 0) {
		myprintk("read u32 array err\r\n");
	}

	for (i = 0; i < 9; ++i) {
		myprintk("reg_values[%d]:0x%x\r\n", i, reg_values[i]);
	}

	ret = of_property_read_string(test_device_node, "status", &str);
	if (ret < 0) {
		myprintk("read string err\r\n");
	}
	myprintk("status:%s\r\n", str);
